					...
					@foreach(auth()->user()->apiKeys as $apiKey)
						...
						<tr class="@if($loop->index%2 == 0){{ 'bg-white' }}@else{{ 'bg-gray-50' }}@endif">
							...
							<td class="px-6 py-4 text-sm font-medium leading-5 text-right whitespace-no-wrap">
								<button onclick="actionClicked('view', '{{ $apiKey->id }}', '{{ $apiKey->name }}', '{{ $apiKey->key }}')" class="mr-2 text-indigo-600 hover:underline focus:outline-none">
									View
								</button>
					...
					<div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
						<div class="flex items-center justify-start mb-4">
							<h3 class="text-lg font-medium leading-6 text-gray-700">
								API Key (<span x-text="name"></span>)
							</h3>
						</div>
						<div class="flex flex-col justify-between w-full mt-2">
							<input type="text" readonly="readonly" onfocus="this.select();" class="px-2 py-1 my-2 font-mono text-gray-700 bg-gray-100 rounded form-input" :value="key" id="viewKeyValue">
			...
			<div x-data="{ open: false, id: '', key: '', name: '' }" x-init="
				...
				<div class="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
					...
					<div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
						<div class="flex items-center justify-start mb-4">
							<h3 class="text-lg font-medium leading-6 text-gray-700">
								API Key Name (<span x-text="name"></span>)
							</h3>
						</div>
						<div class="flex flex-col justify-between w-full mt-2">
							<form action="{{ route('wave.settings.api.put') }}" method="POST">
								<div>
									...
									<div class="mt-1 rounded-md shadow-sm">
										<input id="key_name" type="text" name="key_name" placeholder="Key Name" :value="name" class="w-full form-input">
									</div>
								</div>
								<input type="hidden" name="_method" value="PUT">
								<input type="hidden" name="id" :value="id">
								<p class="flex justify-end mt-5">
									<button x-on:click="open = false" type="button" class="inline-flex justify-center px-4 py-2 mr-3 text-base font-medium leading-6 text-gray-700 transition duration-150 ease-in-out bg-white border border-gray-300 rounded-md shadow-sm hover:text-gray-500 focus:outline-none focus:border-blue-300 focus:shadow-outline sm:text-sm sm:leading-5">Cancel</button>
						...
						<div class="flex flex-col justify-between w-full mt-2">
							<div class="sm:flex sm:items-start">
								...
								<div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
								    ...
								    <div class="mt-2">
										<p class="text-sm leading-5 text-gray-500">Are you sure you want to delete API Key: <code class="px-2 py-1 text-gray-800 bg-gray-200 rounded"><span x-text="name"></span></code></p>
								    </div>
								</div>
							</div>
